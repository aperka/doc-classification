...
from fasttext import fasttext_train, fasttext_get_vectors
...
def extract_features(method, train_data, test_data, train_docs_ids):
    if method == 'tf_idf':
...
def test(classifier_type, feature_extraction, dataset_json):
    for case in range(1, 5):
        ...
        train_docs, train_bin_labels, test_docs, test_bin_labels, labels, train_docs_ids = get_dataset(dataset_json, case)
        train_data_features, test_data_features = extract_features(feature_extraction, train_docs, test_docs, train_docs_ids)
